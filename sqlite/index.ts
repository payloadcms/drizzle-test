...
const start = async () => {
  ...
  db.insert(posts_locales)
    .values([
      { _locale: 'en', title: 'hello', number: 1337, _parentID: post.id, myGroup_subFieldLocalized: 'hello in english', myGroup_subGroup_subSubFieldLocalized: 'sub hello in english' },
      { _locale: 'es', title: 'hola', number: 42069, _parentID: post.id, myGroup_subFieldLocalized: 'hello in spanish', myGroup_subGroup_subSubFieldLocalized: 'sub hello in spanish' },
    ])
    .returning()
    .get()
  ...
  db.insert(posts_my_array_locales)
    .values([
      { _parentID: arrayRows[0].id, subField: 'hello 1', _locale: 'en' },
      { _parentID: arrayRows[1].id, subField: 'hello 2', _locale: 'en' },
    ])
    .returning()
    .get()
